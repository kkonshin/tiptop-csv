(function () {
    ...
    // Обертка для показа спиннера во время обработки файла

    window._oldFetch = fetch;

    window.fetch = function () {
        const fetchStart = new Event(
            'fetchStart', {'view': document, 'bubbles': true, 'cancelable': false}
        );
        const fetchEnd = new Event(
            'fetchEnd', {'view': document, 'bubbles': true, 'cancelable': false}
        );
        var fetchCall = window._oldFetch.apply(this, arguments);
    ...
    window.BlogBFileDialog.prototype.ShowUploadedFile = function (agent) // event
    {
        ...
        if (uploadResult && (uploadResult.element_id > 0)) {
            ...
            if (window.fetch) {
                ...
                fetch('/test/tiptop-csv/process.php', init).then(function (response) {
                    return response.json();
                }).then(function (jsonFilePath) {
                    var pathToFile = '<div style="font-size: 2rem;"><a href="' + jsonFilePath + '">Скачать файл с ценами</a><p style="font-size: 1.6rem;">Файл нужно открывать через Open Office, кодировка UTF-8, разделители - точка c запятой.</p></div>';
                    var fileExtended = document.querySelector('.file-selectdialog');
                    fileExtended.insertAdjacentHTML('afterEnd', pathToFile);
            ...
            /*
             ...
             request.open('POST', '/test/tiptop-csv/index.php');
             request.send();
             }
             */
